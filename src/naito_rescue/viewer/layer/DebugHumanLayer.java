 ...
 */
public class DebugHumanLayer extends StandardEntityViewLayer<Human> {
    ...
    */
	...
	public void setTargetHuman(Human human){
    ...
    public Shape render(Human h, Graphics2D g, ScreenTransform t) {
        ...
        Shape shape, view, targetCircle;
		int targetCircleSize = SIZE * 2;
        int viewSize = (t.xToScreen(location.first() + viewDistance) - x)*2;
        //int viewSize = viewDistance - x;
        Icon icon = useIcons ? getIcon(h) : null;
        if (icon == null) {
            shape = new Ellipse2D.Double(x - SIZE / 2, y - SIZE / 2, SIZE, SIZE);
            view  = new Ellipse2D.Double(x - (viewSize / 2), 
                                          y - (viewSize / 2), 
                                          viewSize, 
                                          viewSize);
